...
void getTags_Lines(){
    ...
    for(int j=0;j<tagCounter;j++){
        ...
        if(lines[i][p2+1] != '<'){
            ...
            t_l.push_back("~"+lines[i].substr(p2+1,temp-p2-1));
        }
  ...
  for(unsigned long long i=0;i<t_l.size();i++){
  ...
  for(unsigned long long i=0;i<tags.size();i++){
    if(!tags[i].empty()){
        if(tags[i].find(' ') != std::string::npos){
            std::string s = tags[i].substr(0,tags[i].find(' '));
            pTags.push_back(s);
        }else{
...
void makePTagsWithoutSlash(){
        ...
        for(unsigned int i=0;i<tags_lines.size();i++){
            if(tags_lines[i][0] == '/'){
...
Node* Tree(std::vector<std::string> withoutSlash,Node* curr_root){
  ...
  for(unsigned int i=0;i<withoutSlash.size();i++){     //makes tree

        if(i==0){
            curr_root = newNode(withoutSlash[i]);
            curr_root->parent = NULL;
            std::stringstream check1(withoutSlash[i]);
            std::string s;
            getline(check1, s, ' ');
            temp.push(s);
...
void printNode(Node* root){
   if(root->data == "\*"){
        ...
        if(root->parent->insideData.empty()&& (root->data[root->data.length()-1] != '}' || root->data[root->data.length()-1] != ']')){
            ...
            json+=root->data;
        }else if(root->parent->insideData.empty() && root->parent->child.size()==1 && (root->data[root->data.length()-1] != '}' || root->data[root->data.length()-1] != ']')){
            json+=root->data;
        }else if(!root->parent->insideData.empty() && root->parent->child.size()==1 && (root->data[root->data.length()-1] != '}' || root->data[root->data.length()-1] != ']')){
            json+="\"text\":"+root->data+"},";
        }
        ...
        if(root->insideData.empty()){
            json+=root->data+":";
        }else if(!root->insideData.empty() && root->child.size()==1){
